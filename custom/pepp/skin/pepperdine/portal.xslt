...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en">
   ...
   <head>
    ...
    <meta http-equiv="Content-Style-Type" content="text/css" />
      ...
      <script type="text/javascript" language="JavaScript" src="/library/js/toolMenuLinks.js">
         <xsl:value-of select="' '"/>
      </script>
   </head>
...
<div>
   <xsl:attribute name="id">
	<xsl:choose>
      		<xsl:when test="currentUser">siteNavWrapper</xsl:when>
		<xsl:otherwise>siteNavWrapperLoggedOut</xsl:otherwise>
	</xsl:choose>
   </xsl:attribute>
   ...
   <div id="mastHead">
      ...
      <div id="mastLogin">
        ...
        <xsl:when test="currentUser">
           <div id="loginLinks">
              <span id="loginUser"><xsl:value-of select="currentUser/first"/><xsl:text> </xsl:text><xsl:value-of select="currentUser/last"/> <xsl:text> | </xsl:text> </span>
              <a target="_parent">
                 <xsl:attribute name="href">
                    <xsl:value-of select="config/logout"/>
                 </xsl:attribute>
        ...
        <xsl:when test="loginInfo/topLogin != 'true'">
			...
			<a target="_parent">
				<xsl:attribute name="href">
                 ...
                 <a target="_parent">
                   <xsl:attribute name="href">/xsl-portal/xlogin</xsl:attribute>
                   <xsl:attribute name="title">Guest Login</xsl:attribute>
                 <xsl:text>Guest Login</xsl:text>
                 </a>
        </div>
        </xsl:when>
        <xsl:otherwise>
...
<br/>
            ...
            <div id="siteNav">
                     ...
                     <xsl:if test="/portal/sites/tabsMoreSites/site">
                        <li class="more-tab">
                           <a href="#" onclick="return dhtml_more_tabs();">
                              ...
                              <span class="outer"><span class="inner"><xsl:text>All My Sites</xsl:text></span></span>
                           </a>
                        </li>
                     </xsl:if>
                     <li style="display:none;border-width:0" class="fixTabsIE"><a href="javascript:void(0);">#x20;</a></li>

                     <li class="edit_tabs"><a title="Edit tabs" alt="Edit tabs">
                       <xsl:attribute name="href">/xsl-portal/directtool/sakai.preferences?sakai.site=~<xsl:value-of select="/portal/currentUser/id" /></xsl:attribute>
                       Edit Tabs
                     </a></li>
               ...
               <xsl:for-each select="/portal/sites/tabsSites/site">
                 <xsl:if test="@selected='true'">
                   <xsl:if test="viewAsStudentLink">
                     <div id="roleSwitch">
                     <xsl:if test="viewAsStudentLink/roleSwitchState='false'">
                       <label for="roleSwitchSelect">View Site As:</label>
                       <a>
                         <xsl:attribute name="href"><xsl:value-of select="viewAsStudentLink/switchRoleUrl"/></xsl:attribute>
                       </a>
                       ...
                       <xsl:for-each select="viewAsStudentLink/siteRoles/role">
                         <option>
                           <xsl:attribute name="value"><xsl:value-of select="../../switchRoleUrl"/><xsl:value-of select="." /><xsl:value-of select="../../panelString"/></xsl:attribute>
                           <xsl:value-of select="." />
                         </option>
                       </xsl:for-each>
                       </select>
                     </xsl:if>
                     <xsl:if test="viewAsStudentLink/roleSwitchState='true'">
                       <a id="roleSwitchAnchor">
                         <xsl:attribute name="href"><xsl:value-of select="viewAsStudentLink/switchRoleUrl" /></xsl:attribute>
                         Exit <xsl:value-of select="viewAsStudentLink/roleUrlValue"/> View
                       </a>
                     </xsl:if>
                     </div>
                   </xsl:if> 
   ...
   <!-- child/parent nav here -->
   <xsl:for-each select="/portal/sites/tabsSites/site">
     <xsl:if test="@selected='true' and @child='true'">
       <ul id="siteHierarchy">
         <li><a><xsl:attribute name="href">/xsl-portal/site/<xsl:value-of select="parent" /></xsl:attribute>
            PARENT</a></li>
         <li><span class="breadSeparator">&gt;</span>
           <a><xsl:attribute name="href">/xsl-portal/site/<xsl:value-of select="siteId" /></xsl:attribute>
         ...
         <li>
            <a>
               ...
               <xsl:if test="@popUp='true'">
                  ...
                  <xsl:attribute name="onclick">window.open('<xsl:value-of select="popUrl"/>','WebContentPopup','resizable=yes,toolbar=no,scrollbars=yes, width=800,height=600')</xsl:attribute>
               </xsl:if>
               <xsl:attribute name="accesskey">
                  <xsl:value-of select="../../@order" />
               </xsl:attribute>
...
<!-- an error with this on prod, but not dev
...
<div class="divColor" id="toolMenuWrap">
	<div id="worksiteLogo">
      <xsl:if test="/portal/config/@pageNavPublished = 'false'">
         <p id="siteStatus">Unpublished Site<br />
         <a href="#">
           <xsl:attribute name="onclick">
             <xsl:choose>
               <xsl:when test="contains(/portal/config/extra/currentUrlPath, 'page')">
                 publishSite('<xsl:value-of select="substring-before(substring-after(/portal/config/extra/currentUrlPath, 'xsl-portal/site/'), '/')"/>');return false;
               </xsl:when>
               <xsl:otherwise>
                 publishSite('<xsl:value-of select="substring-after(/portal/config/extra/currentUrlPath, 'xsl-portal/site/')"/>');return false;
               </xsl:otherwise>
             </xsl:choose>
           </xsl:attribute>
           (Publish Now)
   ...
   <xsl:apply-templates select=".">
      <xsl:with-param name="content" select="'false'"/>
         ...
         <li>
               ...
               <xsl:attribute name="onclick">
                  window.open('/xsl-portal/help/main','Help','resizable=yes,toolbar=no,scrollbars=yes, width=800,height=600')
               </xsl:attribute>
      ...
      <a>
         ...
         <xsl:if test="$currentTool/@popUp='true'">
            ...
            <xsl:attribute name="onclick">
               ...
               <!-- window.open('<xsl:value-of select="$currentTool/url"/>', -->
                  '<xsl:value-of disable-output-escaping="yes" select="title"/>',
